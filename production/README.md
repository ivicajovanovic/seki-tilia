...
Pre dizajna pročitaj `brand/design-system.md`, `brand/design-references/references.json` i `brand/design-references/catalog.md`, pa otvori sve četiri odobrene reference. Njihove stabilne ASCII identifikatore doslovno koristi u `generated/design-direction.json`, ali u `referenceFiles` navedi samo stvarno primenjene reference. U njemu zabeleži `authorId`, jednu familiju renderer-a, najmanje dve korišćene dizajnerske osobine, jedinstvenu `signature`, najmanje dve `designInterventions`, `freshInterventionNote`, `motionTreatment` za Reels i razliku u odnosu na poslednje tri objave. U `input.json` obavezno popuni `contentApproach` i `copyFreshnessNote`, a u `formatAdaptations` objasni stvarnu adaptaciju svakog traženog formata. Za Reels `motionTreatment` mora identično da se upiše i u `video-props.json`, jer renderer na njemu zasniva različit raspored sekvenci i način ulaska elemenata. Za logo je obavezno `logoSurface: "none"`, bez pravougaone podloge, a za tipografiju rendererova Manrope porodica `AUSekiManrope` bez zamenskog fonta.
...
Pravougaoni strukturni elementi u rendereru, uključujući panel, footer, karticu, proizvodnu podlogu i okvir, moraju imati oštre uglove. Zaobljenje je rezervisano isključivo za pill-dugme/kratku ponudnu oznaku i kružni dekorativni oblik. Senke i zamućenja, vektorski ili rasterski, nisu dozvoljeni. Podijum mora čitljivo podržati proizvod, sa izraženim vektorskim gradijentom i prednjom masom koja ulazi iza footera. Pre-flight proverava da renderer ne uvodi nedozvoljeno zaobljenje, senku ili blur.
