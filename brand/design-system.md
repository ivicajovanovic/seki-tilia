...
11. Feed, Story i Reels Hero prikazuju ponudu, identitet proizvoda, najmanje jednu potvrđenu informaciju o proizvodu, rok, dostupnost i čitljiv brend-završetak. Reels Hook prikazuje ceo, dominantan proizvod unutar kadra, bez odsecanja na donjoj ivici; Reels Closing koristi znatno veću slobodnu produktnu masu od pratećeg teksta, bez podijuma.
...
- `videoTemplate` je nezavisna odluka od `motionTreatment`. Vrednost `reel-v1` je podrazumevana. `reel-v2` se koristi samo na eksplicitan korisnički zahtev za taj naziv stila i mora biti potvrđen kroz `input.requestedVideoStyle`. Njegova fiksna motion gramatika je: 10 sekundi, 24 fps, jedan kumulativni 9:16 kadar, početni horizontalni push/wipe, trajni slobodni glavni vizuel desno bez postolja, čisti slide-up/fade tekstovi bez trail kopija ili treperenja, panel sa ulaskom sleva, stagger lista, jedna outline spring lokacijska ikona i najmanje 30 završnih frejmova za čitanje. Pozadina, glavni vizuel i sav tekst potiču iz aktuelnog briefa i palete; referentni sadržaj se nikada ne prenosi.
- U oba Reels šablona veliki promotivni tekst obavezno koristi descender-safe line-height i najmanje `0.12em` donjeg unutrašnjeg prostora. Slova sa donjim potezima, naročito `g`, `j`, `p`, `q` i `y`, ne smeju biti odsečena ni tokom ulaska ni u stabilnom stanju.
- `reel-v2` tekstualni slide-up koristi isključivo celobrojne piksel pozicije; subpikselno transliranje teksta nije dozvoljeno jer može izazvati rasterizaciono podrhtavanje pri kraju easing-a. Čim ulazak završi, wrapper mora potpuno ukloniti `translate` i `opacity` iz inline stila, umesto da ih zadrži na vrednostima `0` i `1`, kako Chromium ne bi nastavio kompozitovanje i menjao antialiasing ivica između frejmova. Lokalni `latin` i `latin-ext` Manrope font-face moraju imati eksplicitne, međusobno određene `unicode-range` vrednosti. Njihovo registrovanje pod istim nazivom bez tih opsega izaziva naizmeničnu rasterizaciju teksta između parnih i neparnih frejmova. Sve korišćene težine 600, 700 i 800 moraju biti učitane pre nastavka rendera. Reels MP4 se renderuje sa `CRF 12` radi sprečavanja kompresionog shimmera na stabilnoj tipografiji.
