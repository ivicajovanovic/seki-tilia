...
**Poseban video template `reel-v2`** aktivira se isključivo kada korisnik eksplicitno napiše da želi stil `reel-v2`. Agent tada upisuje `requestedVideoStyle: "reel-v2"` u `input.json`, `videoTemplate: "reel-v2"` u `video-props.json` i `generated/design-direction.json`, i popunjava ceo `reelV2` objekat samo činjenicama i copy-jem aktuelnog briefa. Bez tog eksplicitnog izraza ostaje `reel-v1`; agent ne sme sam da zaključi da bi `reel-v2` bio prikladan. `reel-v2` je desetosekundni, 24 fps, 9:16 kumulativni kadar sa trajnim glavnim vizuelom, čistim sukcesivnim slide-up/fade ulascima bez duplih tragova ili treperenja, panelom koji ulazi sleva, stagger listom, jednom outline spring lokacijskom ikonom i završnim holdom. Proizvod u ovom posebnom template-u uvek stoji slobodno, bez postolja ili podijuma; to je eksplicitni izuzetak od pravila podijuma za standardni Reels Hero. Kod ovog stila mirni intervali između kumulativnih ulazaka i završni hold namerni su deo motion gramatike; dinamiku nose raspoređeni ulasci do završnog holda, dok već prikazan tekst ostaje stabilan. Ne preuzima temu, identitet, osobu, tekst, medicinski kontekst niti bilo koji podatak iz referentnog videa na osnovu kojeg je motion gramatika analizirana.
