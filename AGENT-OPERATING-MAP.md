...
- Podrazumevani Reels koristi `reel-v1`. `reel-v2` se bira samo posle doslovnog, eksplicitnog korisničkog zahteva za stil `reel-v2`; tada se zahtev evidentira u `input.requestedVideoStyle`, a isti `videoTemplate` upisuje u props i design-direction. Renderer `SekiTiliaReelV2` preuzima isključivo aktuelne podatke iz `reelV2` props objekta.
